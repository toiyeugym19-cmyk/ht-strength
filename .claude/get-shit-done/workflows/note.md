...
- **Global scope**: `D:/gym 2/tap gyp/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/gym 2/tap gyp/.claude/notes/`)
...
2. Glob `D:/gym 2/tap gyp/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/gym 2/tap gyp/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/gym 2/tap gyp/.claude/notes/` — works in any directory
