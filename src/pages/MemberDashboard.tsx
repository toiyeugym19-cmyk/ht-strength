...
export default function MemberDashboard() {
    ...
    const motivationText =
        sessionPct >= 85 ? 'Gần đạt mục tiêu! 💪' :
            sessionPct >= 50 ? 'Đi đúng hướng! 🎯' :
                'Mỗi buổi đều đáng giá! 🔥';
        ...
        >
            ...
            <motion.div variants={fadeUp} className="pt-4 pb-2">
                ...
                <h1 className="text-[32px] font-bold mt-1 leading-tight tracking-tight" style={{ letterSpacing: -0.8 }}>
                    {member?.name || user?.displayName || 'Hội viên'}
                </h1>
            ...
            {/* ── SUPER DASHBOARD: Integrated Focus ── */}
            <motion.div variants={fadeUp} className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                ...
                <Link to="/work" className="gym-card p-4 flex flex-col justify-between superapp-tap-scale overflow-hidden relative group">
                    ...
                    <div>
                        <p className="text-[10px] font-bold uppercase tracking-wider mb-2 opacity-50">Việc ưu tiên</p>
                        {todayFocus ? (
                            <>
                                ...
                                <div className="flex items-center gap-1.5 mt-2">
                                    ...
                                    <span className="text-[11px] font-medium opacity-70">{todayFocus.category === 'work' ? 'Công việc' : 'Cá nhân'}</span>
                                </div>
                            </>
                        ) : (
                            <>
                                <h3 className="text-[15px] font-bold opacity-40">Mọi thứ đã xong!</h3>
                                <p className="text-[11px] mt-1 opacity-30">Hôm nay thật tuyệt vời</p>
                            </>
                        )}
                    </div>
                </Link>
                ...
                <Link to="/my-workout" className="gym-card p-4 flex flex-col justify-between superapp-tap-scale overflow-hidden relative group border-l-4 border-l-[#BF5AF2]">
                    ...
                    <div>
                        <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: 'var(--g-accent-2, #BF5AF2)' }}>Buổi tập tiếp</p>
                        {activeWorkout ? (
                            <>
                                ...
                                <div className="flex items-center gap-1.5 mt-2">
                                    ...
                                    <span className="text-[11px] font-medium opacity-70">Bắt đầu ngay</span>
                                </div>
                            </>
                        ) : (
                            <h3 className="text-[15px] font-bold opacity-40">Chưa có giáo án</h3>
                        )}
                    </div>
                </Link>
            </motion.div>
            ...
            {sessionAlert?.urgent && (
                <motion.div variants={fadeUp} className={`mb-4 gym-alert ${sessionsRemaining === 0 ? 'gym-alert--danger' : 'gym-alert--warning'}`}>
                    ...
                    <p className="text-[14px] font-semibold flex-1" style={{ color: sessionsRemaining === 0 ? '#FF453A' : '#FF9F0A' }}>
                        {sessionsRemaining === 0 ? 'Đã hết buổi — liên hệ PT/Admin để gia hạn' : `Sắp hết buổi — ${sessionAlert.text}`}
                    </p>
                </motion.div>
            )}

            {/* ── SESSION RING HERO CARD ── */}
            <motion.div variants={fadeUp} className="mb-4">
                <div className="gym-card gym-card--strip rounded-[20px] p-5 flex items-center gap-5" style={{ borderColor: `${ringColor}25` }}>
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-[17px] font-bold leading-tight">Buổi thứ {computed.sessionsUsed} trong gói {computed.sessionsTotal}</p>
                        <p className="text-[13px] mt-1 font-medium" style={{ color: 'var(--ios-secondary)' }}>
                            Còn {sessionsRemaining} buổi
                        </p>
            ...
            {/* ── HÔM NAY: 1 việc chính ── */}
            <motion.div variants={fadeUp} className="gym-section">
                <p className="gym-section__title">Hôm nay</p>
                <Link to="/my-workout" className="gym-hero-cta">
                    ...
                    <div className="gym-hero-cta__body">
                        <p className="gym-hero-cta__title">Vào buổi tập</p>
                        <p className="gym-hero-cta__sub">
                            {assignedPlan ? `${assignedPlan.name} · ~${assignedPlan.estimatedMinutes} phút` : 'Chọn giáo án · tick set · hoàn thành'}
                        </p>
                    </div>
            ...
            <motion.div variants={fadeUp} className="mb-4">
                <div className="grid grid-cols-2 gap-2.5">
                    {[
                        { icon: FlameIcon, value: computed.streak, label: 'Streak', unit: 'ngày', color: '#FF453A', bg: 'rgba(255,69,58,0.12)' },
                        { icon: Award, value: computed.attendance30, label: '30 ngày', unit: '%', color: '#BF5AF2', bg: 'rgba(191,90,242,0.12)' },
                        { icon: Calendar, value: computed.joinDays, label: 'Thành viên', unit: 'ngày', color: '#FF9F0A', bg: 'rgba(255,159,10,0.12)' },
                        { icon: Lightning, value: motivationText, label: 'Ghi chú', color: '#0A84FF', bg: 'rgba(10,132,255,0.1)', isText: true },
                    ].map((s, i) => (
            ...
            {/* ── TUẦN NÀY ── */}
            <motion.div variants={fadeUp} className="gym-section">
                <p className="gym-section__title">Tuần này</p>
            ...
            {/* ── KHÁM PHÁ (EXPOSED HUB) ── */}
            <motion.div variants={fadeUp} className="gym-section">
                <p className="gym-section__title">Tiện ích & Khám phá</p>
                <div className="flex flex-col gap-3">
                    <NavCard to="/my-workout" icon={Barbell} label="Buổi tập" desc="Bắt đầu tập luyện ngay hôm nay" color="#FF9F0A" />
                    <NavCard to="/my-plan" icon={ListChecks} label="Kế hoạch" desc="Theo dõi lịch trình & dinh dưỡng" color="#0A84FF" />
                    <NavCard to="/calendar" icon={CalendarBlank} label="Lịch trình" desc="Quản lý sự kiện và lịch hẹn" color="#5E5CE6" />
                    <NavCard to="/calories" icon={FlameIcon} label="Calories" desc="Kiểm soát Kcal nạp & tiêu thụ" color="#FF453A" />
                    <NavCard to="/steps" icon={PersonSimpleRun} label="Bước chân" desc="Theo dõi hoạt động leo thang" color="#30D158" />
                    <NavCard to="/meditation" icon={Brain} label="Thiền định" desc="Thư giãn và tập trung tâm trí" color="#8B5CF6" />
                    <NavCard to="/journal" icon={Bookmark} label="Nhật ký" desc="Ghi chép cảm xúc hàng ngày" color="#BF5AF2" />
                    <NavCard to="/profile" icon={UserCircle} label="Hồ sơ" desc="Thông tin cá nhân & mục tiêu" color="#6366F1" />
                </div>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="mt-12 mb-10 pb-8 px-0">
                ...
                >
                    ...
                    <span className="text-[15px] font-black text-[#FF453A] uppercase tracking-widest">Đăng xuất tài khoản</span>
                </button>
